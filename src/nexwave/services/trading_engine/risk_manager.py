...
class RiskManager:
    """Manages risk limits and order validation"""
    ...
    def __init__(self):
        ...
        self.max_order_size_usd = settings.max_order_size_usd
        self.maintenance_margin_ratio = settings.maintenance_margin_ratio
        self.min_profit_target_usd = settings.min_profit_target_usd
        self.trade_cooldown_seconds = settings.trade_cooldown_seconds
        self.max_trades_per_symbol_per_day = settings.max_trades_per_symbol_per_day

        # Symbol blacklist - worst performers from trade history analysis
        self.symbol_blacklist: Set[str] = {
            s.strip().upper() for s in settings.symbol_blacklist.split(",") if s.strip()
        }

        # Trade throttling tracking
    ...
    def check_trade_frequency(self, symbol: str) -> RiskCheckResult:
        ...
        if symbol_upper in self.last_trade_time:
            ...
            if time_since_last_trade < self.trade_cooldown_seconds:
                ...
                return RiskCheckResult(
                    approved=False,
                    reason=f"Trade cooldown active for {symbol}: {remaining:.0f}s remaining (min {self.trade_cooldown_seconds}s between trades)",
                )

        # Check daily trade limit (max 10 trades per symbol per day)
        daily_count = self.daily_trade_count.get(symbol_upper, 0)
        if daily_count >= self.max_trades_per_symbol_per_day:
            return RiskCheckResult(
                approved=False,
                reason=f"Daily trade limit reached for {symbol}: {daily_count}/{self.max_trades_per_symbol_per_day}",
            )
    ...
    def record_trade(self, symbol: str) -> None:
        ...
        logger.debug(
            ...
            f"{self.daily_trade_count[symbol_upper]}/{self.max_trades_per_symbol_per_day} today"
        )
    ...
    def check_profit_viability(self, order_amount: float, order_price: float) -> RiskCheckResult:
        ...
        min_profit_needed = self.min_profit_target_usd + estimated_fees
        ...
        if required_move_pct > 5.0:
            return RiskCheckResult(
                approved=False,
                reason=f"Trade requires unrealistic {required_move_pct:.2f}% price move for ${self.min_profit_target_usd} profit (after fees)",
                details={
    ...
    async def check_leverage(
        ...
        if leverage > self.max_leverage:
            return RiskCheckResult(
                approved=False,
                reason=f"Leverage too high: {leverage:.2f}x > {self.max_leverage:.2f}x",
